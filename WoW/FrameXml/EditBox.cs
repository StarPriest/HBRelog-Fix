    ...
    public class EditBox : Frame, IFontInstance
    {
		...
		public int CursorPosition
        {
            get
            {
                ...
                var bytePos = WowManager.Processsharp.Memory.Read<int>(Address + Offsets.EditBox.AsciiCursorPositionOffset);
                // calculate position in a utf8 string.
                return text.Take(bytePos).Count();
            }
        }

        public EditBoxFlags EditBoxFlags => (EditBoxFlags)WowManager.Processsharp.Memory.Read<int>(Address + Offsets.EditBox.FlagsOffset);
        ...
        /// The max bytes.
        /// </value>
        public int MaxBytes => WowManager.Processsharp.Memory.Read<int>(Address + Offsets.EditBox.MaxBytesOffset) + 1;
        ...
        /// The max letters.
        /// </value>
        public int MaxLetters => WowManager.Processsharp.Memory.Read<int>(Address + Offsets.EditBox.MaxLettersOffset);
        ...
        public string Text
        {
            get
            {
                var ptr = WowManager.Processsharp.Memory.Read<IntPtr>(Address + Offsets.EditBox.TextOffset);
                if (ptr == IntPtr.Zero) return string.Empty;
                var maxBytes = MaxBytes;
                var maxLetters = MaxLetters;
                return WowManager.Processsharp.Memory.Read(ptr, Encoding.UTF8, maxBytes > 0 ? maxBytes : maxLetters * 4);
            }
        }
	    ...
	    public bool IsEnabled => (WowManager.Processsharp.Memory.Read<int>(Address + Offsets.EditBox.IsEnabledFlagOffset) & Offsets.EditBox.IsEnabledBit) == 0;
