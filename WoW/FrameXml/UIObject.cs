...
namespace HighVoltz.HBRelog.WoW.FrameXml
{
    public class UIObject
    {
        ...
        public string Name
        {
            get
            {
                if (!_triedGetName)
                {
                    ...
                    var ptr = WowManager.Processsharp.Memory.Read<IntPtr>(Address + idx);
                    _name = ptr != IntPtr.Zero ? WowManager.Processsharp.Memory.Read(ptr, Encoding.UTF8, 128) : "<unnamed>";
                    _triedGetName = true;
                }
                return _name;
            }
        ...
        private static bool IsValidTypePtr(ProcessSharp PS, IntPtr ptr)
        {
            if (ptr == IntPtr.Zero) return false;
            try
            {
                var bytes = PS.Memory.Read(ptr, 6);
        ...
        {
            var vtmPtr = wowManager.Processsharp.Memory.Read<IntPtr>(address);
            if (!UIObjectTypeCache.ContainsKey(vtmPtr))
                SetObjectType(wowManager.Processsharp, vtmPtr);
